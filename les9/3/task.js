class Autocomplete {
  ...
  getMatches( text ) {
    ...
    for (let i=0; i<this.input.length;i++) {
      if (Array.from(text).join('')==Array.from(this.input[i].textContent).slice(0, text.length).join('')) {
        result.push({text:this.input[i].textContent,value:this.input[i].value})
      }
    }
    /*
